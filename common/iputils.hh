...
private:
  ...
  public:
     ...
     TreeNode* make_left() {
       if (!left) {
         left = std::unique_ptr<TreeNode>(new TreeNode(d_bits+1));
         left->parent = this;
       }
       return left.get();
     }
     ...
     TreeNode* make_right() {
       if (!right) {
         right = std::unique_ptr<TreeNode>(new TreeNode(d_bits+1));
         right->parent = this;
       }
       return right.get();
     }
     ...
     std::unique_ptr<node_type> node4; //<! IPv4 value-pair
     std::unique_ptr<node_type> node6; //<! IPv6 value-pair
...
public:
  ...
  node_type& insert(const key_type& key) {
    ...
    if (!root) root = std::unique_ptr<TreeNode>(new TreeNode(0));
