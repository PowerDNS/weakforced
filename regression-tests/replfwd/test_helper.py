...
class ApiTestCase(unittest.TestCase):
    ...
    def removeSibling(self, host, port):
        ...
        payload['sibling_port'] = port
        return self.session.post(
            self.url3("/?command=removeSibling"),
            data=json.dumps(payload),
            headers={'Content-Type': 'application/json'})

    def setSiblings(self, payload):
        return self.session.post(
            self.url3("/?command=setSiblings"),
            data=json.dumps(payload),
            headers={'Content-Type': 'application/json'})

    def removeSiblingReplFwdDest(self, host, port):
        payload = dict()
        payload['sibling_host'] = host
        payload['sibling_port'] = port
        return self.session.post(
            self.url3("/?command=removeReplicationForwarderDest"),
            data=json.dumps(payload),
            headers={'Content-Type': 'application/json'})

    def addSiblingReplFwdDest(self, host, port, protocol, key):
        payload = dict()
        payload['sibling_host'] = host
        payload['sibling_port'] = port
        payload['sibling_proto'] = protocol
        payload['encryption_key'] = key
        return self.session.post(
            self.url3("/?command=addReplicationForwarderDest"),
            data=json.dumps(payload),
            headers={'Content-Type': 'application/json'})

    def setSiblingReplFwdDests(self, payload):
        return self.session.post(
        self.url3("/?command=setReplicationForwarderDests"),
        data=json.dumps(payload),
        headers={'Content-Type': 'application/json'})

    def removeSiblingReplFwdSrc(self, host):
        payload = dict()
        payload['sibling_host'] = host
        return self.session.post(
            self.url3("/?command=removeReplicationForwarderSrc"),
            data=json.dumps(payload),
            headers={'Content-Type': 'application/json'})

    def addSiblingReplFwdSrc(self, host):
        payload = dict()
        payload['sibling_host'] = host
        return self.session.post(
            self.url3("/?command=addReplicationForwarderSrc"),
            data=json.dumps(payload),
            headers={'Content-Type': 'application/json'})

    def setSiblingsReplFwdSrc(self, payload):
        return self.session.post(
            self.url3("/?command=setReplicationForwarderSrcs"),
            data=json.dumps(payload),
            headers={'Content-Type': 'application/json'})
